...
BOOL CMydemoDlg::OnInitDialog()
{
	...
	if(!res)
	{
		MessageBox("≥ı ºªØApplication¥ÌŒÛ");

	}
	res=matlabtovcInitialize();
	if(!res)
	{
		MessageBox("≥ı ºªØLib¥ÌŒÛ");
	}
...
void CMydemoDlg::OnCancel() 
{
	...
	if(!res)
	{
		MessageBox("Ω· ¯≥Ã–Ú¥ÌŒÛ£°");
	}
	matlabtovcTerminate();
}

void CMydemoDlg::OnOK() 
{
